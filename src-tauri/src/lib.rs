...
async fn run_code(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        #[cfg(windows)]
        let bin_path = tmp_dir.join("tbcpc_run.exe");
        #[cfg(not(windows))]
        ...
        if ext == "cpp" {
            ...
            let compile = no_window(Command::new("g++").args([
        ...
        if io_mode == "file" {
            std::fs::write(&inp_path, &input)
                .map_err(|e| format!("Cannot write {}: {}", inp_path.display(), e))?;
        }

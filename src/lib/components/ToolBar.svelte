<script lang="ts">
    import { getContext } from "svelte";
    import { invoke } from "@tauri-apps/api/core";

    const ctx = getContext<any>("tabs");
    const actions = getContext<any>("actions");
    let fontSize = $derived(ctx.fontSize);
    let isDark = $derived(ctx.isDark);
    let tabs = $derived(ctx.tabs);
    let activeId = $derived(ctx.activeId);
    let activeTab = $derived(tabs.find((t: any) => t.id === activeId));
    let inputBuffer = $derived(ctx.inputBuffer);
    let running = $state(false);

    function getLanguage(filename: string): string {
        if (filename.endsWith(".py")) return "python";
        return "cpp";
    }

    async function run() {
        if (!activeTab || running) return;
        running = true;
        try {
            await actions.runCode();
        } finally {
            running = false;
        }
    }
</script>

<div
    class="flex flex-row items-center gap-2 px-3 h-full shrink-0 border-l border-base-300"
>
    <!-- Font size -->
    <div class="join">
        <button
            class="join-item btn btn-sm btn-ghost font-mono"
            onclick={() => ctx.setFontSize(Math.max(8, fontSize - 1))}>−</button
        >
        <span
            class="join-item btn btn-sm btn-ghost font-mono no-animation cursor-default min-w-14 text-center"
        >
            {fontSize}px
        </span>
        <button
            class="join-item btn btn-sm btn-ghost font-mono"
            onclick={() => ctx.setFontSize(Math.min(32, fontSize + 1))}
            >+</button
        >
    </div>

    <div class="w-2"></div>

    <!-- IO mode toggle -->
    {#if activeTab}
        <div class="flex flex-row items-center gap-2">
            <span
                class="text-sm font-mono {activeTab.ioMode === 'stdio'
                    ? 'text-base-content'
                    : 'text-base-content/40'}"
            >
                <strong>Stdio</strong>
            </span>
            <label class="toggle toggle-sm">
                <input
                    type="checkbox"
                    checked={activeTab.ioMode === "file"}
                    onchange={() =>
                        (activeTab.ioMode =
                            activeTab.ioMode === "stdio" ? "file" : "stdio")}
                />
            </label>
            <span
                class="text text-sm font-mono {activeTab.ioMode === 'file'
                    ? 'text-base-content'
                    : 'text-base-content/40'}"
            >
                <strong>File</strong>
            </span>
        </div>
    {/if}

    <div class="w-2"></div>

    <!-- Theme toggle — daisyui swap component with theme-controller -->
    <label class="swap swap-rotate btn btn-sm btn-ghost">
        <input
            type="checkbox"
            class="theme-controller"
            value="latte"
            checked={!isDark}
            onchange={() => ctx.toggleTheme()}
        />
        <!-- sun icon — shown in dark mode -->
        <svg
            class="swap-off h-4 w-4 fill-current"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
        >
            <path
                d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z"
            />
        </svg>
        <!-- moon icon — shown in light mode -->
        <svg
            class="swap-on h-4 w-4 fill-current"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
        >
            <path
                d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"
            />
        </svg>
    </label>

    <div class="w-4"></div>

    <!-- Run -->
    <button
        class="btn btn-sm btn-success font-mono font-bold"
        onclick={run}
        disabled={running || !activeTab}
    >
        {#if running}
            <span class="loading loading-spinner loading-xs"></span>
        {:else}
            ▶ Run
        {/if}
    </button>
</div>
